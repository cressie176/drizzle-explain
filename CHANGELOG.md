...
- `allowOperations` entries can carry conditions, so an exemption applies to individual plan nodes rather than the whole plan: `allowOperations: [{ operation: Operation.SEQ_SCAN, maxScanned: 500 }]`. A sequential scan is optimal on a small lookup table and a defect on a large one, and a query joining both previously had to accept or reject the pair together. `relation` names a table and `maxScanned` sets a size above which the exemption lapses, and they combine; `maxActualRows` follows below. Naming a bare operation behaves exactly as before, though it is deprecated in this release. Exemptions are annotated in the failure message, `✓ allowed by maxScanned=500`, so an exempted scan is visible rather than merely absent, though a plan where nothing else failed still produces no message. A node whose driver reported no `scanned` count is never exempted by `maxScanned`, and an entry naming no operation, or a condition that does not exist, is rejected rather than interpreted.
- A `maxActualRows` condition on `allowOperations` entries, exempting nodes that produced at most that many rows. It is the size condition for operations that read no table, joins and sorts, where rows produced is the only size there is; for a scan prefer `maxScanned`, since a scan of 20,000 rows returning one has an `actualRows` of 1 and `maxActualRows` would wave through the very plan you are hunting.
- `OperationExemption` is exported from the type definitions.
- `PlanNode` carries `relation`, `alias` and `scanned` where the database reports them, and the failure message renders all three. A plan joining two tables previously reported two identical `Seq Scan` lines with nothing to tell them apart, and its metrics misled: `estimatedRows` and `actualRows` both count rows *produced*, so a scan reading 20,000 rows to return one showed `rows=1 actual=1` and looked smaller than the 40-row lookup table beside it. Lines now read `Seq Scan on books  (cost=359 estimated=1 actual=1 scanned=20000 time=0.663ms)`, and the summary names the table too. Waste is `scanned` minus the rows the node produced, so the two being equal means the scan threw nothing away. PostgreSQL reports the table and alias separately, so an aliased scan renders as `Seq Scan on books b`; MariaDB reports only the alias once a query uses one, so `relation` carries whichever name it gave.

### Fixed

- `mariadbDriver` accepts a connection pool. It called `beginTransaction` on whatever it was given, so a `mysql2` pool failed with `client.beginTransaction is not a function`, while `postgresDriver` had accepted either a client or a pool since 1.0. The driver now leases a connection for the run and returns it afterwards, matching PostgreSQL, and `mariadb.d.ts` types the argument as `Connection | Pool` rather than `unknown`, so the mistake is caught at compile time as well.
- Limits are validated before the query runs rather than while its plan is analysed, so an unrecognised operation or a malformed exemption fails without touching the database, matching the statement-count assertions.

- An operation `drizzle-explain` does not recognise is rejected rather than ignored. `disallowOperations: ['SEQ SCAN']`, an underscore dropped, matched no node and passed, leaving a test that read as though it asserted something and asserted nothing. Both lists are now validated when the limits are read, which also fixes the narrower case where a malformed `allowOperations` entry was only rejected on a plan that happened to contain a disallowed node, so the error depended on the data rather than on the test.

- `scanned` counts the rows a node read across **every** execution. Both databases report row counts per execution, and a node on the inner side of a nested loop runs once per outer row, so a repeatedly-executed scan described a single pass and understated its work by that factor. A sequential scan executed 200 times, reading 100 rows each, reported `scanned` of 100 against the 20,000 rows it actually read, and an `allowOperations` entry of `{ operation: SEQ_SCAN, maxScanned: 500 }` exempted it. An unindexed inner side of a nested loop is the pathology a scan ban exists to catch, so the exemption was defeating the check on precisely the plan it was set to find. `PlanNode` gained `loops`, rendered as `loops=` when greater than one, and `estimatedRows` and `actualRows` stay per execution so `rowEstimateTolerance` is unchanged. **A `maxScanned` threshold that passed against a repeatedly-executed node may now fail**; the node was always reading that many rows.
- `Operation` can be imported as a named ESM export: `import { createExplain, Operation } from 'drizzle-explain'`, the form every operations example in the README uses, threw `does not provide an export named 'Operation'`. The package is CommonJS, and `lib/index.js` assigned its exports as `{ name: require('./x').name }` pairs, a shape Node's named-export detection reads only partially: it found `createExplain` and missed `Operation`, leaving it reachable only through the default export or a CommonJS `require`. Since `Operation` is the only way to name a value for `disallowOperations` or `allowOperations`, the whole operations feature was unreachable from an ESM consumer following the documentation.
...
- MariaDB plans keep the access node of a scalar subquery. `subqueries` and `subquery_cache`, the wrappers MariaDB nests a select-list subquery under, were missing from the translator alongside the join and sort wrappers below, so the subquery's table never reached the tree. The dropped node is executed once per outer row, which is the shape a scan ban most wants to see.
...
- MariaDB reports `estimatedRows` and `actualRows` as rows *produced*, matching PostgreSQL. Its plans count rows *read* (`rows`, `r_rows`) and report the proportion surviving the filter separately (`filtered`, `r_filtered`), and the driver previously used the read counts directly. A query returning one row out of a 20,000-row scan reported `actualRows` of 20000. The driver now multiplies the counts by their percentages, rounds to whole rows, and keeps the read count for `scanned`, which the entry above turns into a total across executions; where a plan reports no percentage the raw counts stand. **This makes `rowEstimateTolerance` stricter on MariaDB**, and suites that passed may now fail: comparing two read counts hid every selectivity misestimate, because both sides were the same pre-filter number. A full scan whose filter matched one row of 20,000 gave a ratio of 1.0 and passed; it now reports the optimizer's real error, 19761x. Plans where the optimizer was right, an indexed lookup or an unfiltered scan, are unaffected.
