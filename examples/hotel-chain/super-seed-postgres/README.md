...
What differs is the seeding path. This package generates the dataset with [drizzle-super-seed](https://www.npmjs.com/package/drizzle-super-seed) and streams it straight into psql as COPY blocks, cutting the seed of the same million-reservation dataset from 14.6 to 3.1 seconds on one measured machine:
...
2. `seed.ts` recreates the tables, then runs `generate` with `createPostgresSqlStreamSink` piped into a spawned psql. It uses psql from your PATH when present (honouring `PGHOST` and friends), and otherwise falls back to the psql inside the repo's compose service, so no local client install is needed. One consequence: with `PGHOST` and friends pointing at a remote database, a local psql is required, because the fallback client only reaches the compose service.
