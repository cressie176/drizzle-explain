...
function postgresDriver(client, config) {
  return { explain: (run) => explainStatements(client, config, run) };
}

function explainStatements(client, config, run) {
  return withRolledBackTransaction(client, async (connection) => {
    const statements = [];
    const sequencer = createStatementSequencer();
    const db = instrumentedDatabase(connection, config, statements, sequencer);
    await runToCompletion(run, db, sequencer);
    return statements;
  });
}

function instrumentedDatabase(connection, config, statements, sequencer) {
  const db = drizzle(transparentProxy(connection, statements, sequencer), config);
  return supportTransactions(db, (sql) => connection.query(sql), sequencer);
}
