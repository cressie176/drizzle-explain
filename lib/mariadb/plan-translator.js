...
function translateTable(table) {
  ...
  assignNumber(node, 'loops', table.r_loops);
  assignNumber(node, 'scanned', rowsScanned(table));
...
function rowsScanned(table) {
  if (typeof table.r_rows !== 'number') return undefined;
  return Math.round(table.r_rows * loopsOf(table));
}

function loopsOf(table) {
  if (typeof table.r_loops !== 'number') return 1;
  return table.r_loops;
}
...
const childBuilders = {
  ...
  subqueries: (value) => value.flatMap(childrenOf),
  subquery_cache: (value) => childrenOf(value),
};
