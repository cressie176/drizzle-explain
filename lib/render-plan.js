...
const metrics = [
  ...
  { read: (node) => repeated(node.loops), format: (value) => `loops=${value}` },
...
const repeated = (loops) => (isPresent(loops) && loops > 1 ? loops : undefined);

function renderSummary(breaches) {
  return breaches.map((breach) => summaryFormatters[breach.limit](breach)).join('\n');
}

function groupAnnotationsByNode(entries) {
  const annotationsByNode = new Map();
  for (const { node, text } of entries) {
    const annotations = annotationsByNode.get(node) ?? [];
    annotations.push(text);
    annotationsByNode.set(node, annotations);
  }
  return annotationsByNode;
}

function annotationsOf(analysis) {
  return [
    ...analysis.breaches.map((breach) => ({ node: breach.node, text: annotationFormatters[breach.limit](breach) })),
    ...(analysis.exemptions ?? []).map((exemption) => ({ node: exemption.node, text: renderExemption(exemption) })),
  ];
}

function renderExemption({ exemption }) {
  return `✓ ${describeConditions(exemption)}`;
}

function describeConditions(entry) {
  const conditions = conditionsOf(entry).map(([key, value]) => `${key}=${value}`);
  if (conditions.length === 0) return 'allowed';
  return `allowed by ${conditions.join(', ')}`;
}

function conditionsOf(entry) {
  if (typeof entry !== 'object' || entry === null) return [];
  return Object.entries(entry).filter(([key]) => key !== 'operation');
}

function renderMetrics(node) {
  const parts = metrics.filter((field) => isPresent(field.read(node))).map((field) => field.format(field.read(node)));
...
function renderPlan(root, analysis) {
  ...
  const tree = renderTree(root, groupAnnotationsByNode(annotationsOf(analysis)));
  return `${summary}\n\n${tree}`;
}
